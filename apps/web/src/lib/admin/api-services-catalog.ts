...
export const API_SERVICES_CATALOG: ApiServiceEntry[] = [
  ...
  },
  {
    ...
    tier: 'paid',
    category: 'AI',
    description: 'MISA chatbot, Admin AI Chat, WhatsApp bot brain, Call IQ. Blog AI uses OpenRouter.',
  ...
  },
  {
    ...
    tier: 'paid',
    category: 'AI',
    description: 'AI Write, FAQs, tags, and local SEO for blogs only.',
    billingModel: 'Per token via OpenRouter',
    adminMenus: ['Digital Marketing Blogs', 'AI Written Blogs'],
    envKeys: ['OPENROUTER_API_KEY', 'OPENROUTER_BLOG_MODEL', 'OPENROUTER_HTTP_REFERER'],
  ...
  },
  {
    ...
    tier: 'paid',
    category: 'Messaging',
    description: 'Optional SMS fallback for invoices when DLT gateway is not used.',
    billingModel: 'Per SMS if TWILIO_* env is set',
    adminMenus: ['Invoice send API (background)', 'DLT SMS (Twilio provider)'],
    envKeys: ['TWILIO_ACCOUNT_SID', 'TWILIO_AUTH_TOKEN'],
  },
  {
    id: 'dlt-sms',
    name: 'DLT SMS (TRAI / Jio TrueConnect)',
    tier: 'paid',
    category: 'Messaging',
    description: 'India DLT SMS: PE, headers, templates, CTA — send via your own operator HTTP pipe (no aggregator).',
    billingModel: 'Operator SMS tariff after you register as telemarketer on Jio/Airtel',
    adminMenus: ['DLT Dashboard', 'Entity', 'Header SMS', 'Content Templates', 'Own SMS pipe', 'Send SMS'],
    dashboardHref: '/dashboard/super_admin/dlt-sms',
    docsUrl: 'https://trueconnect.jio.com',
  },
  {
    id: 'clarity',
