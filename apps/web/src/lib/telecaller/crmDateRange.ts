...
/** Build UTC ISO from an IST calendar date + HH:mm (phone timezone nahi). */
export function istDateTimeToIso(ymd: string, hm: string): string | null {
  const date = String(ymd || '').slice(0, 10);
  const time = String(hm || '').slice(0, 5);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(time)) return null;
  const d = new Date(`${date}T${time}:00+05:30`);
  return Number.isNaN(d.getTime()) ? null : d.toISOString();
}

export function isoToIstParts(iso: string | null | undefined): { ymd: string; hm: string } | null {
  if (!iso) return null;
  const d = new Date(String(iso));
  if (Number.isNaN(d.getTime())) return null;
  return { ymd: istYmd(d), hm: istHm(d) };
}
