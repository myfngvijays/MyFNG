...
export async function recordDpdpConsents(rows: ConsentRecordInput[]): Promise<{ stored: boolean; error?: string }> {
  ...
  try {
    ...
    if (!supabaseAdmin) return { stored: false, error: adminError || 'Supabase admin unavailable' };
    ...
    const { error } = await supabaseAdmin.from('dpdp_consent_records').insert(payload);
