...
async function assertRsaManager(request: NextRequest) {
  const supabase = await createClientFromRequest(request);
  const {
    data: { user },
    error: authError,
  } = await supabase.auth.getUser();
  if (authError || !user) return { ok: false, status: 401, error: 'Unauthorized', supabase };

  const profile = await resolveUserProfile(supabase as any, user as any);
  const roleCode = (profile?.roles as any)?.role_code || null;
  if (roleCode !== 'RSA_MANAGER') return { ok: false, status: 403, error: 'Forbidden', supabase };

  return { ok: true, status: 200, error: null, supabase };
}

export async function GET(request: NextRequest) {
  try {
    const auth = await assertRsaManager(request);
