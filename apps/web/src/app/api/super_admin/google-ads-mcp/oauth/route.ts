...
async function requireAdsAccess(request: NextRequest) {
  ...
  if (!auth.ok) return { ok: false as const, status: auth.res.status || 403, error: 'Forbidden' };
  return { ok: true as const, userId: auth.user.id, roleCode: auth.roleCode };
}

function adsPanel(origin: string, roleCode?: string) {
  return roleCode === 'DIGITAL_MARKETING'
    ? `${origin}/dashboard/digital_marketing/google-ads`
    : `${origin}/dashboard/super_admin/google-ads-mcp`;
}
...
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    ...
    const panel = adsPanel(origin, gate.roleCode);
    if (!oauth.clientId) {
