  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: hint || 'Failed to update promo banner', details: error.message, code, hint: (error as any).hint },
        { status: 500 }
      );
    }
    return NextResponse.json({ data, message: 'Promo banner updated successfully' });
  } catch (e: any) {
...
export async function DELETE(_request: NextRequest, { params: paramsPromise }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (error) {
      ...
      const hint = code === '42P01' || /does not exist/i.test(error.message) ? MIGRATION_HINT : undefined;
      return NextResponse.json(
        { error: hint || 'Failed to delete promo banner', details: error.message, code, hint: (error as any).hint },
        { status: 500 }
      );
    }
