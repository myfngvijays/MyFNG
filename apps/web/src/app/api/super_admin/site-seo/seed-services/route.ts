import { SERVICE_PAGE_SEO_DEFAULTS } from '@/lib/service-page-seo';
import {
...
export async function POST() {
  try {
    ...
    const missing = SERVICE_PAGE_SEO_DEFAULTS.filter((row) => !existingPaths.has(row.page_path));
    ...
    if (error) {
      return NextResponse.json(
        {
          error: 'Failed to seed service page SEO',
