...
import {
  normalizePublicFaqBulkInput,
  publicFaqBulkToInsert,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const faqGroup = normalizePublicFaqGroup(body?.faq_group);
    const sectionKey = String(body?.section_key || 'general').trim() || 'general';
    const sectionTitle = String(body?.section_title || 'General').trim() || 'General';
    const rawRows = Array.isArray(body?.faqs) ? body.faqs : [];
    if (rawRows.length === 0) {
      return NextResponse.json({ error: 'No FAQs provided for bulk upload' }, { status: 400 });
    }
    if (rawRows.length > MAX_BULK) {
      return NextResponse.json({ error: `Maximum ${MAX_BULK} FAQs per bulk upload` }, { status: 400 });
    }

    const rows = [];
    const errors: string[] = [];
    rawRows.forEach((raw: Record<string, unknown>, index: number) => {
      const normalized = normalizePublicFaqBulkInput(raw, index + 1);
      if (!normalized) {
        errors.push(`Row ${index + 1}: question and answer are required.`);
        return;
      }
      rows.push(publicFaqBulkToInsert(normalized, faqGroup, sectionKey, sectionTitle));
    });

    if (rows.length === 0) {
      return NextResponse.json({ error: 'No valid FAQs to import', errors }, { status: 400 });
    }

    const { data, error } = await db.from(PUBLIC_FAQS_TABLE).insert(rows).select('id');
    if (error) {
    ...
    return NextResponse.json({
      ...
      errors,
      message: `Imported ${data?.length || rows.length} FAQ(s)`,
    });
  } catch (e: any) {
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const body = await request.json().catch(() => ({}));
    const updates = Array.isArray(body?.updates) ? (body.updates as PublicFaqBulkUpdate[]) : [];
    ...
    if (updates.length > 0) {
      ...
      for (const row of updates) {
        ...
        const mapped = mapPublicFaqRow(existing);
        const payload: Record<string, unknown> = { updated_at: new Date().toISOString() };
        if (row.question) payload.question = row.question;
        if (row.answer) payload.answer = row.answer;
        ...
        const { error } = await db.from(PUBLIC_FAQS_TABLE).update(payload).eq('id', id);
        if (error) {
    ...
    const { data: existing, error: fetchError } = await db.from(PUBLIC_FAQS_TABLE).select('*').in('id', ids);
    if (fetchError) {
      return NextResponse.json({ error: 'Failed to load FAQs', details: fetchError.message }, { status: 500 });
    }

    let updated = 0;
    for (const row of existing || []) {
      const mapped = mapPublicFaqRow(row);
      const next = {
        visible_android:
      ...
      const { error } = await db.from(PUBLIC_FAQS_TABLE).update(buildVisibilityUpdate(next)).eq('id', mapped.id);
      if (!error) updated += 1;
    }
    ...
    return NextResponse.json({
      success: true,
      updated,
      message: `Updated visibility for ${updated} FAQ(s)`,
    });
  } catch (e: any) {
    return NextResponse.json({ error: e?.message || 'Internal server error' }, { status: 500 });
  }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error } = await db.from(PUBLIC_FAQS_TABLE).delete().in('id', ids);
    if (error) {
    ...
    return NextResponse.json({
      success: true,
      deleted: ids.length,
      message: `Deleted ${ids.length} FAQ(s)`,
    });
  } catch (e: any) {
    return NextResponse.json({ error: e?.message || 'Internal server error' }, { status: 500 });
  }
