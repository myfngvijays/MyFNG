...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if lead is validated
    if (lead.status !== 'VALIDATED') {
      return NextResponse.json(
        { error: `Cannot assign workshop. Lead must be VALIDATED first. Current status: ${lead.status}` },
        { status: 400 }
      );
    }
    ...
    await supabase.from('lead_activities').insert({
      lead_id,
      user_id: user.id,
      activity_type: 'WORKSHOP_ASSIGNED',
      description: `Lead assigned to workshop: ${workshop.name}`,
    ...
    await supabase.from('lead_status_history').insert({
      ...
      new_status: 'ASSIGNED_TO_WORKSHOP',
      changed_by: user.id,
      reason: 'Workshop Assignment',
      notes: `Assigned to ${workshop.name}. ${assignment_notes || ''}`
    });
    ...
    await supabase.from('lead_events').insert({
      ...
      event_description: `Lead ${lead.lead_number} has been assigned to your workshop`,
      event_data: {
        workshop_id,
        workshop_name: workshop.name,
