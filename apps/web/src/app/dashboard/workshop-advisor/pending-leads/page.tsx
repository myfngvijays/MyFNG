...
export default function WorkshopSupervisorPendingLeadsPage() {
  ...
  const { workshopId: sessionWorkshopId } = useAdvisorSession();
  ...
  async function fetchPendingLeads() {
    ...
    try {
      const workshop = sessionWorkshopId;
      if (!workshop) {
  ...
  return (
    <DashboardLayout role="workshop_supervisor">
      <div className="mx-auto w-full max-w-6xl min-w-0 space-y-3 overflow-x-hidden pb-8 sm:space-y-4">
        <AdvisorPageHeader
          title="Lead Approval"
          subtitle="Accept or reject incoming leads. Accept assigns you as advisor."
          href="/dashboard/workshop-advisor/pending-leads"
        />
              ...
              <div key={lead.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-bold text-slate-900">#{lead.lead_number}</p>
                ...
                <p className="truncate text-xs text-slate-500">{lead.customer_phone}</p>
                <p className="mt-1 truncate text-xs text-slate-600">{lead.vehicle_number} · {lead.vehicle_make} {lead.vehicle_model}</p>
                <p className="mt-1 truncate text-xs text-slate-600">{lead.service_type_names || lead.service_type}</p>
                {lead.sla_accept_deadline ? (
                  <p className="mt-2 rounded-lg bg-orange-50 px-2 py-1 text-[11px] font-semibold text-orange-700">
                    SLA {formatDateTime(lead.sla_accept_deadline)}
                  </p>
                ) : null}
                <div className="mt-3 grid grid-cols-3 gap-2">
                  <button
                    onClick={() => router.push(`/dashboard/workshop-advisor/jobs/${lead.id}`)}
                    className="inline-flex min-h-11 items-center justify-center rounded-xl bg-blue-600 text-xs font-bold text-white"
                  >
                    View
                  </button>
                  <button
                    ...
                    className="inline-flex min-h-11 items-center justify-center rounded-xl bg-emerald-600 text-xs font-bold text-white disabled:opacity-50"
                  >
                    Accept
                  </button>
                  <button
                    ...
                    disabled={processing === lead.id}
                    className="inline-flex min-h-11 items-center justify-center rounded-xl bg-red-600 text-xs font-bold text-white disabled:opacity-50"
                  >
                    Reject
                  </button>
                </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  ...
                  <button
                    onClick={() => router.push(`/dashboard/workshop-advisor/jobs/${lead.id}`)}
                            className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded-lg font-semibold inline-flex items-center gap-1"
                  >
                  ...
                  <button
                    onClick={() => handleAcceptLead(lead.id)}
                    disabled={processing === lead.id}
                            className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs rounded-lg font-semibold inline-flex items-center gap-1 disabled:opacity-50"
                  >
                    {processing === lead.id ? (
                  ...
                  <button
                    onClick={() => handleRejectLead(lead.id)}
                    disabled={processing === lead.id}
                            className="px-2.5 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded-lg font-semibold inline-flex items-center gap-1 disabled:opacity-50"
                  >
