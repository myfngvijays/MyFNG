...
import {
  TrendingUp, Users, Building, CheckCircle, XCircle,
  Clock, AlertTriangle, BarChart3, Calendar, Download,
  Loader2, ArrowLeft, ArrowUpRight, PieChart
} from 'lucide-react';
import { toast } from 'react-hot-toast';

export default function LeadManagerPipelineReportsPage() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/telecaller/crm/reports/pipeline?${params}`);
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || 'Failed to load pipeline report');
      ...
      setStatusBreakdown(Array.isArray(json.status_breakdown) ? json.status_breakdown : []);
      setWorkshopPerformance(Array.isArray(json.workshop_performance) ? json.workshop_performance : []);
      setCityDistribution(Array.isArray(json.city_distribution) ? json.city_distribution : []);
      setDailyTrends(Array.isArray(json.daily_trends) ? json.daily_trends : []);
      setPriorityDistribution(Array.isArray(json.priority_distribution) ? json.priority_distribution : []);
      setSlaStats(
        json.sla || {
          on_time: 0,
          at_risk: 0,
  ...
  useEffect(() => {
    void fetchReports();
    // initial load only — Apply button re-runs when user changes dates
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const exportReport = () => {
    window.open(
      `/api/telecaller/crm/reports/export?kind=leads&period=month&date=${dateRange.end}`,
      '_blank',
    );
    ...
    return (
      <DashboardLayout role="lead_manager">
        <div className="flex items-center justify-center h-48 sm:h-64 md:h-screen">
          <Loader2 className="w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 animate-spin text-brand-primary" />
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout role="lead_manager">
      <div className="w-full min-w-0 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            <div className="min-w-0 flex-1">
              <Link
                ...
                className="inline-flex items-center gap-1 text-xs font-bold text-blue-100 hover:text-white mb-2"
              >
                <ArrowLeft className="w-3.5 h-3.5" /> Back to Reports
              </Link>
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold">Pipeline analytics</h1>
              <p className="text-blue-100 font-medium text-xs sm:text-sm md:text-base mt-0.5 sm:mt-1">
                Validation, workshop mix, city split, and SLA
              </p>
            </div>
            <button
              onClick={exportReport}
              className="bg-white text-[#023D95] px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold flex items-center gap-1.5 sm:gap-2 hover:bg-blue-50 transition w-full sm:w-auto justify-center"
            >
        ...
        {/* Date Range Filter */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 sm:p-4 mb-4 sm:mb-5 md:mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 md:gap-4">
            ...
            <button
              ...
              className="rounded-xl bg-[#004AAD] px-3 py-1.5 text-xs font-bold text-white"
            >
              Apply
            </button>
          </div>
