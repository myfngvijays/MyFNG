...
export default function CreatedManualInvoicesClient() {
  ...
  const basePath = '/dashboard/super_admin/manual-invoices/created';
  ...
  }, [page, pageSize, fromDate, toDate, q]);

  const returnTo = useMemo(() => {
    return currentQueryString ? `${basePath}?${currentQueryString}` : basePath;
  }, [basePath, currentQueryString]);

  // Initialize state from URL (so back/refresh keeps filters)
  useEffect(() => {
    const sp = searchParams;
    const nextPage = Math.max(1, parseInt(sp.get('page') || '1', 10) || 1);
    const nextPageSize = Math.min(500, Math.max(10, parseInt(sp.get('pageSize') || '25', 10) || 25));
    const nextFrom = sp.get('fromDate')?.trim() || '';
    const nextTo = sp.get('toDate')?.trim() || '';
    const nextQ = sp.get('q')?.trim() || '';

    // Only update when different to avoid loops.
    setPage((p) => (p !== nextPage ? nextPage : p));
    setPageSize((ps) => (ps !== nextPageSize ? nextPageSize : ps));
    setFromDate((v) => (v !== nextFrom ? nextFrom : v));
    setToDate((v) => (v !== nextTo ? nextTo : v));
    setQ((v) => (v !== nextQ ? nextQ : v));
    setInitialized(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams]);

  // Keep URL updated with current filters/pagination
  useEffect(() => {
    if (!initialized) return;
    const nextUrl = currentQueryString ? `${basePath}?${currentQueryString}` : basePath;
    router.replace(nextUrl);
  }, [initialized, currentQueryString, basePath, router]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialized, page, pageSize, fromDate, toDate, q]);
    ...
    <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-5 sm:py-6 md:py-8 space-y-4">
      <div className="flex items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-600 mt-1">All invoices created via CSV import or single-create.</p>
        </div>
      ...
      <div className="bg-white rounded-lg border p-4">
        <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:justify-between">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full">
            <label className="text-xs text-gray-600 flex flex-col gap-1 sm:col-span-3">
              Search (Invoice # / Name / Mobile)
              <input
                className="border rounded-md px-2 py-2 text-sm"
                placeholder="e.g. RA-614, Yash, 9999999999"
                value={q}
                onChange={(e) => {
                  setQ(e.target.value);
                  setPage(1);
                ...
                <tr key={inv.id} className="border-t">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3 flex-wrap">
                      <Link
                        href={`/dashboard/super_admin/manual-invoices/${inv.id}/edit?returnTo=${encodeURIComponent(returnTo)}`}
