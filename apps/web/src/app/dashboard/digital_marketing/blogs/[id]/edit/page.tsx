...
type Version = { id: string; version_number: number; updated_at: string };
...
export default function DigitalMarketingEditBlogPage() {
  ...
  async function fetchVersions() {
    try {
      const res = await fetch(`/api/blogs/${blogId}/versions`);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) return;
  ...
  async function restoreVersion(versionId: string) {
    const ok = confirm('Restore this version? Current changes will be overwritten.');
    if (!ok) return;
    try {
      const res = await fetch(`/api/blogs/${blogId}/versions`, {
        method: 'POST',
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {versions.slice(0, 4).map((v) => (
                  <button key={v.id} type="button" className="btn btn-outline btn-sm" onClick={() => restoreVersion(v.id)}>
                    <RotateCcw className="w-4 h-4" /> v{v.version_number}
                  </button>
                ))}
              </div>
            </div>
