...
export default function DigitalMarketingProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      await Promise.all([loadProfile(), loadLoginHistory()]);
      setLoading(false);
    })();
      ...
      <div className="mx-auto max-w-3xl space-y-4 pb-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-slate-500">Account details in the same layout as Telecaller</p>
          </div>
          {!loading && profile ? (
            !isEditing ? (
              <button
            ...
            <section className="overflow-hidden rounded-2xl border border-[#004AAD]/25 shadow-lg shadow-[#004AAD]/15">
              ...
              <div className="grid grid-cols-2 gap-3 bg-[#F0F7FF] px-4 py-4 sm:px-5 text-sm border-t border-[#004AAD]/10">
                <div>
                  ...
                  <p className="font-bold text-[#023D95]">{profile?.role?.role_name || 'Digital Marketing'}</p>
                </div>
                <div>
                  <p className="text-xs font-bold text-[#004AAD]/70">Role Code</p>
                  <p className="font-bold text-[#023D95]">{profile?.role?.role_code || 'DIGITAL_MARKETING'}</p>
                </div>
                <div>
