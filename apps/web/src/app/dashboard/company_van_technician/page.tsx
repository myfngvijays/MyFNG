...
export default function CompanyVanTechnicianDashboard() {
  ...
  async function fetchStats() {
    ...
    try {
      ...
      console.error('Failed to load technician stats', e);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <DashboardLayout role="company_van_technician">
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-text-heading">Van Technician</h1>
          <p className="text-sm text-gray-600 mt-1">My HOME_SERVICE tasks</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Link className="card p-4 hover:shadow-md transition" href="/dashboard/company_van_technician/tasks">
            <div className="text-base font-semibold">My Tasks</div>
            <div className="text-xs text-gray-600">Assigned jobs</div>
          </Link>
          <Link className="card p-4 hover:shadow-md transition" href="/dashboard/company_van_technician/history">
