      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Customer Service Executive - Lead Details</p>
          </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="text-lg font-semibold">{lead.customer_name}</div>
                    <div className="text-sm text-gray-500">{lead.customer_phone}</div>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="text-sm text-gray-600">Vehicle</div>
                    <div className="text-lg font-semibold">{lead.vehicle_number || 'N/A'}</div>
                    <div className="text-sm text-gray-500">{lead.vehicle_model || 'N/A'}</div>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <div className="text-sm text-gray-600">Workshop</div>
                    <div className="text-lg font-semibold">{lead.workshop?.name || 'N/A'}</div>
                    <div className="text-sm text-gray-500">{lead.workshop?.city || 'N/A'}</div>
                  </div>
                </div>
                {lead.internal_notes && (
                  <div className="bg-yellow-50 p-4 rounded-lg border-l-4 border-yellow-500">
                    <div className="text-sm font-semibold text-yellow-800 mb-2">Internal Notes</div>
                    <div className="text-sm text-yellow-700 whitespace-pre-wrap">{lead.internal_notes}</div>
                  </div>
                )}
              </div>
            )}
                  ...
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <div className="text-lg font-semibold">{lead.customer_phone}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-2">
                        ...
                        <div className="text-lg">{lead.customer_email || 'Not provided'}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-2">
                        ...
                        <div className="text-lg">{lead.customer_address || 'Not provided'}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Communication Preference</label>
                        <div className="text-lg">{lead.communication_preference || 'PHONE'}</div>
                      </div>
                    </div>
                    <button
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="text-lg">{lead.vehicle_model || 'N/A'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Vehicle Make</label>
                    <div className="text-lg">{lead.vehicle_make || 'N/A'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Variant</label>
                    <div className="text-lg">{lead.vehicle_variant || 'N/A'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Fuel Type</label>
                    <div className="text-lg">{lead.fuel_type || 'N/A'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Odometer</label>
                    <div className="text-lg">{lead.odometer_reading ? `${lead.odometer_reading} km` : 'N/A'}</div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="text-lg font-semibold">₹{lead.estimated_cost?.toLocaleString() || lead.estimated_amount?.toLocaleString() || 'N/A'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Workshop</label>
                    <div className="text-lg">{lead.workshop?.name || 'N/A'}</div>
                    {lead.workshop?.phone && (
                      <div className="text-sm text-gray-500">{lead.workshop.phone}</div>
                    )}
                  </div>
                </div>
                {lead.problem_description && (
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-lg">{lead.problem_description}</div>
                  </div>
                )}
                <div className="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500">
                  <p className="text-sm text-blue-800">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="text-lg">{lead.scheduled_pickup_time || 'Not scheduled'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Pickup Address</label>
                    <div className="text-lg">{lead.pickup_address || lead.customer_address || 'N/A'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Pickup Status</label>
                    <div className="text-lg">{lead.pickup_status || 'Pending'}</div>
                  </div>
                  {lead.pickup_otp && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Pickup OTP</label>
                      <div className="text-lg font-mono font-semibold">{lead.pickup_otp}</div>
                    </div>
                  )}
                  {lead.assigned_pickup_boy && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Pickup Boy</label>
                      <div className="text-lg">{lead.assigned_pickup_boy.full_name || 'N/A'}</div>
                      {lead.assigned_pickup_boy.phone && (
                        <div className="text-sm text-gray-500">{lead.assigned_pickup_boy.phone}</div>
                      )}
                    </div>
                  )}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="text-lg">{lead.scheduled_delivery_time || 'Not scheduled'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Delivery Address</label>
                    <div className="text-lg">{lead.delivery_address || lead.customer_address || 'N/A'}</div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Delivery Status</label>
                    <div className="text-lg">{lead.delivery_status || 'Pending'}</div>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {lead.assigned_mechanic && (
                    <div>
                      ...
                      <div className="text-lg">{lead.assigned_mechanic.full_name || 'N/A'}</div>
                    </div>
                  )}
                  {lead.assigned_supervisor && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Supervisor</label>
                      <div className="text-lg">{lead.assigned_supervisor.full_name || 'N/A'}</div>
                    </div>
                  )}
                  {lead.expected_delivery_time && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Expected Delivery</label>
                      <div className="text-lg">{new Date(lead.expected_delivery_time).toLocaleString()}</div>
                    </div>
                  )}
                </div>
                {lead.extra_charges && lead.extra_charges.length > 0 && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Extra Charges Requested</label>
                    <div className="space-y-2">
                      {lead.extra_charges.map((charge: any, idx: number) => (
                        <div key={idx} className="bg-yellow-50 p-3 rounded-lg border-l-4 border-yellow-500">
                          ...
                          <div className="text-sm text-gray-600">Amount: ₹{charge.amount}</div>
                          {charge.reason && <div className="text-sm text-gray-600">Reason: {charge.reason}</div>}
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="text-lg font-semibold">{lead.invoice.invoice_number || 'N/A'}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Final Amount</label>
                        <div className="text-lg font-semibold">₹{lead.invoice.final_amount?.toLocaleString() || 'N/A'}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Base Amount</label>
                        <div className="text-lg">₹{lead.invoice.base_amount?.toLocaleString() || 'N/A'}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Parts Cost</label>
                        <div className="text-lg">₹{lead.invoice.parts_cost?.toLocaleString() || '0'}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Extra Charges</label>
                        <div className="text-lg">₹{lead.invoice.extra_charges_amount?.toLocaleString() || '0'}</div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Tax</label>
                        <div className="text-lg">₹{lead.invoice.total_tax?.toLocaleString() || '0'}</div>
                      </div>
                    </div>
                    <div className="flex gap-2">
              ...
              <div className="space-y-4">
                ...
                {tickets.length === 0 ? (
                  <div className="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-sm text-gray-500 mt-2">Create a ticket to track customer issues</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {tickets.map((ticket) => (
                      <Link
                        ...
                        className="block bg-white p-4 rounded-lg border border-gray-200 hover:border-indigo-500 hover:shadow-md transition-all"
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <div className="font-semibold text-gray-900">{ticket.ticket_number}</div>
                            <div className="text-sm text-gray-600 mt-1">{ticket.title}</div>
