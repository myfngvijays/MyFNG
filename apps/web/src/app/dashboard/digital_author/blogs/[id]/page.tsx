...
export default function BlogDetailPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'published':
        return <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-semibold">Published</span>;
      case 'draft':
        return <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-sm font-semibold">Draft</span>;
      case 'archived':
        return <span className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm font-semibold">Archived</span>;
      default:
          ...
          <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-2">
                ...
                {blog.is_featured && (
                  <span className="px-2 py-1 bg-brand-primary text-white rounded text-xs font-semibold whitespace-nowrap">Featured</span>
                )}
                {blog.is_premium && (
                  <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded text-xs font-semibold whitespace-nowrap">Premium</span>
                )}
              </div>
            </div>
          </div>
          <div className="flex-shrink-0">
            <Link href={`/dashboard/digital_author/blogs/${blogId}/edit`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-semibold text-sm sm:text-base text-text-heading truncate">{blog.category?.name || 'Uncategorized'}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm text-gray-600">
                  {blog.published_at ? 'Published' : 'Created'}
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  {blog.images.map((img) => (
                    <div key={img.id} className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      />
                      {img.caption && (
                        <p className="p-2 sm:p-3 text-xs sm:text-sm text-gray-600 break-words">{img.caption}</p>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3 text-xs sm:text-sm">
                  ...
                  {blog.seo_data.meta_description && (
                    <div>
                      ...
                      <p className="font-medium text-text-heading line-clamp-2 break-words">{blog.seo_data.meta_description}</p>
                    </div>
                  )}
