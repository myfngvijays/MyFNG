  ...
  return (
    <div className="p-3 sm:p-4 md:p-5 lg:p-6">
      <div className="mb-4 sm:mb-5 md:mb-6">
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-1 sm:mb-2">Chargeback Management</h1>
        <p className="text-gray-600 text-xs sm:text-sm md:text-base">Manage payment disputes and chargebacks</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 sm:py-10 md:py-12 bg-gray-50 rounded-lg">
          <Shield className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 text-green-600 mx-auto mb-2 sm:mb-3 md:mb-4" />
          <p className="text-gray-600 text-xs sm:text-sm md:text-base">No chargebacks found</p>
        </div>
      ) : (
        <div className="grid gap-3 sm:gap-4">
          {chargebacks.map(chargeback => (
            <div key={chargeback.id} className="bg-white p-3 sm:p-4 md:p-5 lg:p-6 rounded-lg border shadow-sm border-l-4 border-l-red-500">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                <div className="flex-1 min-w-0 w-full">
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
                    <AlertOctagon className="w-5 h-5 sm:w-6 sm:h-6 text-red-600 flex-shrink-0" />
                    <h3 className="font-bold text-base sm:text-lg truncate">Case #{chargeback.pg_case_id}</h3>
                    ...
                    {chargeback.priority === 'CRITICAL' && (
                      <span className="px-2 sm:px-3 py-0.5 sm:py-1 bg-red-600 text-white rounded-full text-xs sm:text-sm font-bold">
                        CRITICAL
                      </span>
                    )}
                  </div>
                  ...
                  <div className="p-2.5 sm:p-3 bg-red-50 rounded text-xs sm:text-sm mb-2 sm:mb-3">
                    ...
                    {chargeback.customer_statement && (
                      <>
                        <p className="font-medium mt-1.5 sm:mt-2 mb-0.5 sm:mb-1">Customer Statement:</p>
                  ...
                  {chargeback.our_response && (
                    <div className="p-2.5 sm:p-3 bg-blue-50 rounded text-xs sm:text-sm">
                      <p className="font-medium mb-0.5 sm:mb-1">Our Defense:</p>
                  ...
                  {chargeback.status === 'RECEIVED' && (
                    <>
                      <button
                        ...
                        className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs sm:text-sm w-full sm:w-auto"
                      >
                        Submit Evidence
                      </button>
                      <button className="px-3 sm:px-4 py-1.5 sm:py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-xs sm:text-sm w-full sm:w-auto">
                        View Details
                      </button>
                    </>
                  )}
