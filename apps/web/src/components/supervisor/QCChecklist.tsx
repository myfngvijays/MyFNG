...
import { CheckCircle, XCircle, Loader2, AlertTriangle, Check, X, Camera, Wrench, Image as ImageIcon, FileText } from 'lucide-react';
...
export default function QCChecklist({ 
  ...
  const [serviceChecklistItems, setServiceChecklistItems] = useState<any[]>([]);
  ...
  useEffect(() => {
    async function fetchEvidence() {
      try {
        ...
        // 4) Mechanic service checklist remarks (read-only for QC)
        const { data: svcChecklistRow, error: svcChecklistError } = await supabase
          .from('service_checklists')
          .select('checklist_items')
          .eq('lead_id', leadId)
          .maybeSingle();
        if (!svcChecklistError && svcChecklistRow?.checklist_items) {
          let items: any = svcChecklistRow.checklist_items as any;
          ...
          setServiceChecklistItems(Array.isArray(items) ? items : []);
        } else {
          setServiceChecklistItems([]);
        }
  ...
  return (
    <div className="card">
      ...
      <div className="mb-6 space-y-3">
        ...
        {/* Mechanic remarks from Service Checklist */}
        <div className="p-4 rounded-lg border border-gray-200 bg-white">
          <div className="flex items-center gap-2 mb-2">
            <FileText className="w-5 h-5 text-brand-primary" />
            <h4 className="font-semibold text-gray-800">Mechanic Checklist Remarks</h4>
          </div>
          {serviceChecklistItems.filter((it: any) => String(it?.remark || '').trim()).length === 0 ? (
            <p className="text-sm text-gray-600">No remarks added by mechanic.</p>
          ) : (
            <div className="space-y-2">
              {serviceChecklistItems
                .filter((it: any) => String(it?.remark || '').trim())
                .slice(0, 20)
                .map((it: any, idx: number) => (
                  <div key={`${it?.id || idx}`} className="p-2 rounded bg-gray-50 border border-gray-200">
                    <div className="text-sm font-semibold text-gray-800">{it?.name || it?.item_name || 'Checklist Item'}</div>
                    <div className="text-sm text-gray-700 mt-0.5">
                      <span className="font-semibold">Remark:</span> {String(it.remark)}
                    </div>
                  </div>
                ))}
              {serviceChecklistItems.filter((it: any) => String(it?.remark || '').trim()).length > 20 && (
                <p className="text-xs text-gray-500">Showing first 20 remarks…</p>
              )}
            </div>
          )}
        </div>
