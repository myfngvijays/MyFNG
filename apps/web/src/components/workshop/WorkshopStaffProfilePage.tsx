...
export default function WorkshopStaffProfilePage({
  layoutRole,
  roleFallback,
  extra,
}: {
  ...
  const sessionProfile = useAuthStore((s) => s.userProfile);
  const [profile, setProfile] = useState<any>(sessionProfile);
  const [ready, setReady] = useState(!!sessionProfile);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        .from('users_login')
        .select('*, role:role_id(role_name, role_code), workshop:workshops(name, city, address, state, pincode)')
        .eq('id', user.id)
        .maybeSingle();
      if (cancelled) return;
      let profileRow = data;
      ...
      if (profileRow) {
        ...
        const parts =
          profileRow.first_name || profileRow.last_name
            ? { first_name: String(profileRow.first_name || '').trim(), last_name: String(profileRow.last_name || '').trim() }
            ...
            <section className="overflow-hidden rounded-2xl border border-[#004AAD]/25 shadow-lg shadow-[#004AAD]/15">
              ...
              <div className="grid grid-cols-2 gap-3 border-t border-[#004AAD]/10 bg-[#F0F7FF] px-4 py-4 text-sm sm:px-5">
                <div>
                  ...
                  <p className="font-bold text-[#023D95]">{profile?.role?.role_name || roleFallback}</p>
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <p className="font-semibold text-[#023D95]">{workshopLine || workshop?.city || '—'}</p>
                </div>
              </div>
            </section>
              ...
              {loginLoading ? (
                ...
                <ul className="space-y-2">
                  {loginRecent.slice(0, 20).map((row) => (
                    <li key={row.id} className="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2 text-sm">
