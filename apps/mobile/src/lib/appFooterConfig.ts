...
import { DEFAULT_APP_FOOTER_CONFIG, type AppFooterConfig } from './appFooterConfigDefaults';
...
export async function fetchAppFooterConfig(force = false): Promise<AppFooterConfig> {
  ...
  inflight = fetch(`${ENV.API_URL}/api/public/app-footer/config`, { cache: 'no-store' })
    .then(async (res) => {
