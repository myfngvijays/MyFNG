...
export default function CMRSATasksScreen() {
  ...
  return (
    <View style={styles.container}>
      <DashboardHeader title="My RSA Tasks" onBack={() => navigation.goBack()} />
        ...
        {tasks.length === 0 ? (
          ...
          tasks.map((task, index) => (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('CMRSATaskDetail', { taskId: task.id })}
            >
              ...
              {task.service_location && (
                <Text style={styles.location}>📍 {task.service_location}</Text>
              )}
            </TouchableOpacity>
          ))
        )}
