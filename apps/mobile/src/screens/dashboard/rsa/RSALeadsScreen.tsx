...
export default function RSALeadsScreen() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      let query = supabase
        .from('service_leads')
        .select('*, workshop:workshops(name), mechanic:users_login(full_name)')
        .eq('service_type', 'RSA')
      ...
      >
        {leads.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No RSA leads found</Text>
          </View>
        ) : (
          leads.map((lead, index) => (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('RSALeadDetail', { leadId: lead.id })}
            >
              ...
              {lead.vehicle_details && (
                <Text style={styles.vehicleDetails}>{lead.vehicle_details}</Text>
              )}
