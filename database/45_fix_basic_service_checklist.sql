...
-- Purpose: Ensure checklist items are generated correctly for Basic Service
-- ============================================

-- Update the generate_service_checklist function to better match Basic Service
...
BEGIN
  ...
  CASE 
    ...
    WHEN UPPER(p_service_type) LIKE '%BASIC SERVICE%' OR 
         UPPER(p_service_type) LIKE '%BASIC%15%' OR
         UPPER(p_service_type) LIKE '%BASIC%15 POINTS%' OR
         p_service_type = 'Basic Service (15 Points)' OR
         p_service_type = 'Basic Service' THEN
      v_checklist_items := '[
    ...
    WHEN UPPER(p_service_type) LIKE '%PREMIUM SERVICE%' OR 
         UPPER(p_service_type) LIKE '%PREMIUM%50%' OR
         UPPER(p_service_type) LIKE '%PREMIUM%50 POINTS%' OR
         p_service_type = 'Premium Service (50 Points)' OR
         p_service_type = 'Premium Service' THEN
      v_checklist_items := '[
        {"id": "1", "name": "Clean Air Filter", "status": "PENDING", "mandatory": true, "category": "Engine Compartment", "remark": ""},
...
-- Update the trigger function to ensure checklist is created
...
BEGIN
  ...
  IF NEW.mechanic_id IS NOT NULL AND (OLD IS NULL OR OLD.mechanic_id IS NULL) THEN
    ...
    SELECT EXISTS(
      SELECT 1 FROM service_checklists WHERE lead_id = NEW.lead_id
...
COMMENT ON FUNCTION generate_service_checklist IS 'Generates service checklist based on service type. Supports Basic Service (15 Points) and Premium Service (50 Points) with flexible name matching.';
    ...
    WHERE mj.mechanic_id IS NOT NULL
      AND NOT EXISTS (
        ...
        WHERE sc.lead_id = mj.lead_id
          AND (sc.checklist_items IS NULL OR jsonb_array_length(sc.checklist_items) = 0)
      )
  LOOP
    ...
    DELETE FROM service_checklists WHERE lead_id = v_lead_record.lead_id;
