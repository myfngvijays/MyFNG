-- 222_register_customer_fcm_token_deactivate_stale.sql
-- When a customer registers a new FCM token, deactivate older tokens for the same customer.
-- Fixes "Delivered" to simulator/dev token while TestFlight device never receives push.
...
BEGIN
  ...
  -- Keep only the latest device token per customer (avoids sends to simulator/old installs).
